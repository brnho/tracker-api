...
async function counts(_, { status, effortMin, effortMax }) {
	...
	const results = await db.collection('issues').aggregate([
		...
		{
			$group: {
				_id: { owner: '$owner', status: '$status' }, //now you know why we need '', it's a necessity for js objects
				count: { $sum: 1},
			},
		},
	]).toArray();
	...
	return Object.values(stats); //note that we have produced the exact format we want
}
